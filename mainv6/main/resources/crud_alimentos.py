...
def ler_alimento():
    ...
    if alimentos:
        ...
        for alimento in alimentos:
            print(f"ID: {alimento[0]}, Nome: {alimento[1]}, Tipo: {alimento[2]}, Valor: R${alimento[3]:.2f}")
    else:
...
def loop_menu_crud_alimentos():
    while True:
        ...
        if escolhamenu == 1:
            ...
            tipo = input("Digite o novo tipo (peso/unidade) ou pressione Enter para não alterar: ")
            valor = input("Digite o novo valor ou pressione Enter para não alterar: ")
            valor = float(valor) if valor else None
            atualizar_alimento(nome, tipo or None, valor)
        elif escolhamenu == 4:
            nome = input("Digite o nome do alimento a ser deletado: ")
            deletar_alimento(nome)
        elif escolhamenu == 5:
            fl.limpar()
            conexao.close()
            return
