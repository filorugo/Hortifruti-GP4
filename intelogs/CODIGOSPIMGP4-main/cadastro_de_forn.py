...
conn = sqlite3.connect('fornecedores.db')  # Conecta ao banco de dados (ou cria, se não existir)
...
''')
conn.commit()  # Confirma (commita) a criação da tabela no banco de dados
...
def menu():
    while True:  # Loop principal do menu, repetido até a opção "Sair" ser escolhida
        ...
        if opcao == "1":
            ...
            print("Saindo...")  # Confirmação de saída
            break  # Encerra o loop e o programa
        else:
            print("Opção inválida. Tente novamente.")  # Mensagem para opção inválida

# Executa o menu
menu()  # Inicia o menu para o usuário interagir com o sistema

# Fecha a conexão com o banco de dados
conn.close()  # Fecha a conexão para liberar os recursos do banco de dados
